...
import { TrendingUp, Package, Clock, PauseCircle } from "lucide-react";
  ...
  return (
    <span
      className={`font-mono text-xs ${
        paused ? "text-orange-400" : "text-amber-400"
      }`}
    >
      {label}
    </span>
      ...
      {loading ? (
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {plans.map((plan) => {
            ...
            const gradient =
              config?.color ?? "from-cyan-500 to-blue-600";

            const border =
              config?.border ?? "border-cyan-500/30";
            ...
            return (
              <div
                key={plan.id}
                className={`rounded-2xl border ${border} overflow-hidden bg-white/[0.03]`}
              >
                <div
