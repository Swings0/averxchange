...
import {
  ...
  Package, List, XCircle, Pencil, PauseCircle, PlayCircle,
} from "lucide-react";
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 backdrop-blur-sm px-4">
      <div className="bg-[#0f1f35] border border-white/10 rounded-3xl p-8 max-w-sm w-full shadow-2xl text-center">
        <PauseCircle size={32} className="text-amber-400 mx-auto mb-4" />
        <h2 className="text-lg font-bold text-white mb-2">Pause Plan?</h2>
        <p className="text-white/50 text-sm mb-6">
          Are you sure you want to pause <span className="text-white font-semibold">{planName}</span>?
          The plan progress will be frozen until you resume it.
        </p>
        <div className="flex gap-3">
          <button
            onClick={onCancel}
            className="flex-1 py-3 rounded-xl border border-white/10 text-white/70 hover:bg-white/10 transition text-sm font-medium"
          >
            Cancel
          </button>
          <button
            onClick={onConfirm}
            disabled={loading}
            className="flex-1 py-3 rounded-xl bg-amber-500/20 border border-amber-500/30 text-amber-400 hover:bg-amber-500/30 transition text-sm font-semibold flex items-center justify-center gap-2"
          >
      ...
      <div className="flex items-center gap-1.5 flex-wrap">
        {type === "withdrawal" && tx.status === "pending" && (
          <button onClick={() => handleAction("approve")} disabled={!!acting}
            ...
            {acting === "approve" ? <Loader2 size={10} className="animate-spin" /> : <CheckCircle2 size={10} />} Approve
          </button>
        )}
        {type === "withdrawal" && tx.status === "pending" && (
          <button onClick={() => handleAction("decline")} disabled={!!acting}
            ...
            {acting === "decline" ? <Loader2 size={10} className="animate-spin" /> : <XCircle size={10} />} Decline
          </button>
        )}
        <button onClick={() => handleAction("delete")} disabled={!!acting}
          ...
          {acting === "delete" ? <Loader2 size={10} className="animate-spin" /> : <Trash2 size={10} />} Delete
        </button>
      </div>
    </div>
  );
...
function UserStatsModal({ user, onClose }: { user: any; onClose: () => void }) {
  ...
  const [localStats, setLocalStats] = useState<Record<string, number>>({
    balance: user.balance ?? 0, totalProfit: user.totalProfit ?? 0,
    bonus: user.bonus ?? 0, tradingAccounts: user.tradingAccounts ?? 0,
    referralBonus: user.referralBonus ?? 0, totalDeposit: user.totalDeposit ?? 0,
  ...
  const updateTxStatus = (id: string, newStatus: string, type: "deposit" | "withdrawal") => {
    if (type === "deposit") setLocalDeposits((prev) => prev.map((d) => d.id === id ? { ...d, status: newStatus } : d));
    else setLocalWithdrawals((prev) => prev.map((w) => w.id === id ? { ...w, status: newStatus } : w));
  };
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-start justify-center bg-black/70 backdrop-blur-sm px-4 pt-16 pb-6 overflow-y-auto">
      <div className="bg-[#0f1f35] border border-white/10 rounded-3xl w-full max-w-2xl shadow-2xl my-auto">
        {toast && (
          <div className={`absolute top-4 left-1/2 -translate-x-1/2 z-10 px-5 py-2 rounded-full text-xs font-medium border ${toast.type === "success" ? "bg-emerald-500/20 border-emerald-500/30 text-emerald-300" : "bg-rose-500/20 border-rose-500/30 text-rose-300"}`}>
            {toast.msg}
          </div>
        )}
        <div className="flex items-center justify-between px-6 py-4 border-b border-white/[0.06] sticky top-0 bg-[#0f1f35] rounded-t-3xl z-10">
          ...
          <button onClick={onClose} className="w-9 h-9 flex items-center justify-center rounded-xl bg-white/10 hover:bg-white/20 text-white transition-all flex-shrink-0">
            <X size={16} />
          </button>
        </div>
        <div className="p-6 space-y-6">
          <div>
            ...
            <div className="space-y-2">
              {MODAL_STAT_FIELDS.map(({ key, label }) => (
                <div key={key} className="flex items-center gap-3 bg-white/5 border border-white/[0.06] rounded-xl px-4 py-3">
                    ...
                    {editing[key] !== undefined ? (
                      <div className="flex items-center gap-2">
                        ...
                        <input type="number" value={editing[key]}
                          ...
                          autoFocus className="flex-1 bg-white/10 border border-cyan-500/40 rounded-lg px-2 py-1 text-sm text-white focus:outline-none" />
                        <button onClick={() => handleSave(key)} disabled={saving === key}
                          ...
                          {saving === key ? <Loader2 size={11} className="animate-spin" /> : <Save size={11} />} Save
                        </button>
                        <button onClick={() => setEditing((prev) => { const n = { ...prev }; delete n[key]; return n; })}
                          className="text-xs px-2 py-1 text-white/30 hover:text-white transition"><X size={13} /></button>
                      </div>
                    ) : (
                  ...
                  {editing[key] === undefined && (
                    <div className="flex items-center gap-1.5 flex-shrink-0">
                      <button onClick={() => setEditing((prev) => ({ ...prev, [key]: String(localStats[key]) }))}
                      ...
                      <button onClick={() => handleClear(key)} disabled={saving === `clear-${key}`}
                        ...
                        {saving === `clear-${key}` ? <Loader2 size={10} className="animate-spin" /> : <Trash2 size={10} />} Clear
                      </button>
                    </div>
                  )}
                </div>
                ...
                {user.plans.map((p: any) => (
                  <div key={p.id} className="flex items-center justify-between bg-white/5 border border-white/[0.06] rounded-xl px-4 py-3">
                    <div>
                      ...
                      <p className="text-xs text-white/40">${(p.price ?? 0).toLocaleString()} → +${(p.totalProfit ?? 0).toLocaleString()} · {p.duration}d</p>
                    </div>
          ...
          <div>
            <p className="text-xs text-white/30 uppercase tracking-widest mb-3">Deposits ({localDeposits.length})</p>
            {!localDeposits.length ? (
              <p className="text-sm text-white/30 bg-white/5 rounded-xl px-4 py-3">No deposits</p>
            ) : (
              <div className="space-y-2 max-h-48 overflow-y-auto">
                {localDeposits.map((d) => (
                  <ModalTransactionRow key={d.id} tx={d} type="deposit"
          ...
          <div>
            <p className="text-xs text-white/30 uppercase tracking-widest mb-3">Withdrawals ({localWithdrawals.length})</p>
            {!localWithdrawals.length ? (
              ...
              <div className="space-y-2 max-h-48 overflow-y-auto">
                {localWithdrawals.map((w) => (
                  <ModalTransactionRow key={w.id} tx={w} type="withdrawal"
...
function StatRow({ fieldKey, label, userEmail, onSuccess, onError }: {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      onSuccess(data.message); setValue("");
...
function PlanSlot({ index, onChange }: { index: number; onChange: (v: any) => void }) {
  ...
  const emit = (overrides: any = {}) => {
    const durationSeconds = durationMode === "custom"
      ? (customDuration.days * 86400 + customDuration.hours * 3600 + customDuration.mins * 60 + customDuration.secs) : 0;
    onChange({
        ...
        {open && (
          <div className="absolute top-full left-0 right-0 mt-1 bg-[#0f1f35] border border-white/10 rounded-xl overflow-hidden z-20 shadow-xl max-h-64 overflow-y-auto">
            <button onClick={() => { setPlanId(""); setOpen(false); emit({}); }}
              className="w-full px-4 py-2.5 hover:bg-white/10 transition text-sm text-left text-white/40">No preset / Custom only</button>
            {PLANS.map((p) => (
              <button key={p.id} onClick={() => {
                setPlanId(p.id); setOpen(false);
                if (!customName) setCustomName(p.name);
                setCustomPrice(String(p.price)); setCustomDailyProfit(String(p.dailyProfit));
                setCustomTotalProfit(String(p.totalProfit)); setDurationDays(String(p.duration));
...
export default function AdminDashboard() {
  ...
  const [pausingPlan, setPausingPlan] = useState<string | null>(null);
  ...
  useEffect(() => {
    const fetchDeposits = async () => {
      try { const res = await fetch("/api/admin/deposits"); const data = await res.json(); setDeposits(data.deposits ?? []); } catch {}
      finally { setLoadingDeposits(false); }
    };
    fetchDeposits(); const id = setInterval(fetchDeposits, 30000); return () => clearInterval(id);
  }, []);

  useEffect(() => {
    const fetchWithdrawals = async () => {
      try { const res = await fetch("/api/admin/withdrawals"); const data = await res.json(); setWithdrawals(data.withdrawals ?? []); } catch {}
      finally { setLoadingWithdrawals(false); }
    };
    fetchWithdrawals(); const id = setInterval(fetchWithdrawals, 30000); return () => clearInterval(id);
  }, []);

  const handleWithdrawalAction = async (transactionId: string, action: string) => {
    setProcessingWithdrawal(`${transactionId}-${action}`);
    try {
      const res = await fetch("/api/admin/withdrawals", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ transactionId, action }) });
      const data = await res.json();
      if (!res.ok) { showToast(data.error, "error"); return; }
      showToast(data.message, "success");
      if (action !== "leavePending") setWithdrawals((prev) => prev.filter((w) => w.id !== transactionId));
  ...
  const handleDepositAction = async (transactionId: string, action: "approve" | "decline") => {
    ...
    try {
      const res = await fetch("/api/admin/deposits", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ transactionId, action }) });
      const data = await res.json();
      if (!res.ok) { showToast(data.error, "error"); return; }
      showToast(data.message, "success");
  ...
  const handleSearch = async () => {
    if (!email) return;
    setSearching(true); setShowUserPlans(false);
    try {
      ...
      setUserData(data); setSearchedEmail(email); setUserFound(true); setUserPlans(data.plans ?? []);
  ...
  const handleRemovePlan = async (planId: string) => {
    ...
    try {
      const res = await fetch("/api/admin/remove-plan", { method: "DELETE", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ planId }) });
  ...
  const handlePausePlan = async (planId: string, action: "pause" | "resume") => {
    setPausingPlan(planId);
    try {
      const res = await fetch("/api/admin/pause-plan", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
      ...
      setUserPlans((prev) =>
        prev.map((p) =>
          p.id === planId
            ? { ...p, status: action === "pause" ? "paused" : "active" }
            : p
        )
      );
    } catch { showToast("Network error", "error"); }
    finally { setPausingPlan(null); setPauseTarget(null); }
  };

  const updatePlanCount = (c: number) => {
    const n = Math.max(1, Math.min(5, c));
    setPlanCount(n); setPlanSlots(Array(n).fill(null).map(() => ({})));
  };
  ...
  const handleProcessPlans = async () => {
    ...
    try {
      const res = await fetch("/api/admin/update-plans", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ email: searchedEmail, plans: validPlans, delayMs: toMs(planDelay) }) });
      const data = await res.json();
      if (!res.ok) { showToast(data.error, "error"); return; }
      showToast(data.message, "success");
  ...
  return (
    <div className="min-h-screen bg-[#080f1a] text-white">
      ...
      {pauseTarget && (
        <PauseConfirmModal
          planName={pauseTarget.name}
          onCancel={() => setPauseTarget(null)}
          onConfirm={() => handlePausePlan(pauseTarget.id, "pause")}
          loading={pausingPlan === pauseTarget.id}
        />
      )}

      {toast && (
        <div className={`fixed top-5 left-1/2 -translate-x-1/2 z-50 flex items-center gap-2 px-6 py-3 rounded-full border text-sm font-medium shadow-xl ${toast.type === "success" ? "bg-emerald-500/20 border-emerald-500/30 text-emerald-300" : "bg-rose-500/20 border-rose-500/30 text-rose-300"}`}>
      ...
      <div className="max-w-5xl mx-auto px-4 py-8 space-y-10">
        ...
        <div className="space-y-4">
          <div className="flex items-center gap-3">
            ...
            <h2 className="text-sm font-bold text-white/60 uppercase tracking-widest whitespace-nowrap flex items-center gap-2">
              Pending Deposits
              {deposits.length > 0 && <span className="bg-amber-500/20 text-amber-400 border border-amber-500/30 text-xs px-2 py-0.5 rounded-full">{deposits.length} pending</span>}
            </h2>
            <div className="h-px flex-1 bg-white/[0.06]" />
          </div>
          {loadingDeposits ? (
                ...
                <div key={d.id} className="rounded-2xl border border-amber-500/20 bg-amber-500/5 p-5 flex flex-wrap items-center justify-between gap-4">
                  <div className="space-y-1">
                    ...
                    <p className="text-xs text-white/50">
                      <span className="text-emerald-400 font-bold text-sm">${d.amount?.toLocaleString()}</span>{" "}via {d.paymentMethod}
                      {d.proofFileName && <span className="ml-2 text-white/30">· Proof: {d.proofFileName}</span>}
                    </p>
                    <p className="text-xs text-white/25">{new Date(d.createdAt).toLocaleString()}</p>
                  </div>
                  <div className="flex items-center gap-2">
                    <button onClick={() => handleDepositAction(d.id, "approve")} disabled={!!processingDeposit}
                      ...
                      {processingDeposit === `${d.id}-approve` ? <Loader2 size={12} className="animate-spin" /> : <CheckCircle2 size={13} />} Approve
                    </button>
                    <button onClick={() => handleDepositAction(d.id, "decline")} disabled={!!processingDeposit}
                      ...
                      {processingDeposit === `${d.id}-decline` ? <Loader2 size={12} className="animate-spin" /> : <XCircle size={13} />} Decline
                    </button>
                  </div>
                </div>
              ))}
        ...
        <div className="space-y-4">
          <div className="flex items-center gap-3">
            ...
            <h2 className="text-sm font-bold text-white/60 uppercase tracking-widest whitespace-nowrap flex items-center gap-2">
              Pending Withdrawals
              {withdrawals.length > 0 && <span className="bg-rose-500/20 text-rose-400 border border-rose-500/30 text-xs px-2 py-0.5 rounded-full">{withdrawals.length} pending</span>}
            </h2>
            <div className="h-px flex-1 bg-white/[0.06]" />
          </div>
          {loadingWithdrawals ? (
                ...
                <div key={w.id} className="rounded-2xl border border-rose-500/20 bg-rose-500/5 p-5 space-y-4">
                  <div className="flex flex-wrap items-start justify-between gap-3">
                    <div className="space-y-1">
                      ...
                      <p className="text-xs text-white/50"><span className="text-rose-400 font-bold text-sm">${w.amount?.toLocaleString()}</span>{" "}via {w.method}</p>
                  ...
                  <div className="flex flex-wrap items-center gap-2">
                    <button onClick={() => handleWithdrawalAction(w.id, "approve")} disabled={!!processingWithdrawal}
                      className="flex items-center gap-1.5 px-4 py-2 bg-emerald-500/20 border border-emerald-500/30 text-emerald-400 rounded-xl text-xs font-semibold hover:bg-emerald-500/30 disabled:opacity-40 transition-all">
                      {processingWithdrawal === `${w.id}-approve` ? <Loader2 size={12} className="animate-spin" /> : <CheckCircle2 size={13} />} Approve
                    </button>
                    <button onClick={() => handleWithdrawalAction(w.id, "leavePending")} disabled={!!processingWithdrawal}
                      ...
                      {processingWithdrawal === `${w.id}-leavePending` ? <Loader2 size={12} className="animate-spin" /> : <Clock size={13} />} Leave Pending
                    </button>
                    <button onClick={() => handleWithdrawalAction(w.id, "declineWithEmail")} disabled={!!processingWithdrawal}
                      className="flex items-center gap-1.5 px-4 py-2 bg-rose-500/20 border border-rose-500/30 text-rose-400 rounded-xl text-xs font-semibold hover:bg-rose-500/30 disabled:opacity-40 transition-all">
                      {processingWithdrawal === `${w.id}-declineWithEmail` ? <Loader2 size={12} className="animate-spin" /> : <XCircle size={13} />} Decline + Email
                    </button>
                    <button onClick={() => handleWithdrawalAction(w.id, "declineWithoutEmail")} disabled={!!processingWithdrawal}
                      ...
                      {processingWithdrawal === `${w.id}-declineWithoutEmail` ? <Loader2 size={12} className="animate-spin" /> : <XCircle size={13} />} Decline Silent
                    </button>
                  </div>
                </div>
              ))}
        ...
        {userFound && (
          <div className="space-y-4">
            ...
            <button onClick={() => setShowUserPlans((v) => !v)}
              ...
              User Current Plans
              {userPlans.length > 0 && (
                <span className="ml-1 bg-cyan-500/20 text-cyan-400 text-xs px-2 py-0.5 rounded-full border border-cyan-500/30">{userPlans.length} active</span>
              )}
              ...
              <div className="rounded-2xl border border-white/[0.06] bg-white/[0.03] overflow-hidden">
                {userPlans.length === 0 ? (
                  <div className="flex items-center gap-3 px-5 py-5 text-sm text-white/30">
                    <Package size={16} className="text-white/20" /> User has no active plans
                  </div>
                ) : (
                    ...
                    {userPlans.map((p) => (
                      <div key={p.id} className="flex items-center justify-between px-5 py-4 hover:bg-white/[0.02] transition">
                        <div>
                          ...
                          <p className="text-xs text-white/25">Ends: {new Date(p.endDate).toLocaleDateString()}</p>
                        </div>
                        <div className="flex items-center gap-2">
                          {/* ── NEW: Pause / Resume button ── */}
                          {p.status === "paused" ? (
                            <button
                              onClick={() => handlePausePlan(p.id, "resume")}
                              disabled={pausingPlan === p.id}
                              className="flex items-center gap-1.5 px-3 py-2 bg-emerald-500/20 border border-emerald-500/30 text-emerald-400 rounded-xl text-xs font-semibold hover:bg-emerald-500/30 disabled:opacity-40 transition-all"
                            >
                              {pausingPlan === p.id ? <Loader2 size={12} className="animate-spin" /> : <PlayCircle size={12} />}
                              Resume
                            </button>
                          ) : (
                            <button
                              onClick={() => setPauseTarget({ id: p.id, name: p.name })}
                              disabled={pausingPlan === p.id}
                              className="flex items-center gap-1.5 px-3 py-2 bg-amber-500/20 border border-amber-500/30 text-amber-400 rounded-xl text-xs font-semibold hover:bg-amber-500/30 disabled:opacity-40 transition-all"
                            >
                              <PauseCircle size={12} /> Pause
                            </button>
                          )}
                          <button onClick={() => handleRemovePlan(p.id)} disabled={removingPlan === p.id}
                            ...
                            {removingPlan === p.id ? <Loader2 size={12} className="animate-spin" /> : <Trash2 size={12} />} Remove
                          </button>
                        </div>
                      </div>
                    ))}
            ...
            <div className="flex items-center gap-3">
              ...
              <div className="flex items-center gap-2">
                <button onClick={() => updatePlanCount(planCount - 1)} className="w-8 h-8 rounded-lg bg-white/10 border border-white/10 flex items-center justify-center hover:bg-white/20 transition text-white"><Minus size={14} /></button>
                <span className="text-white font-bold w-6 text-center">{planCount}</span>
                <button onClick={() => updatePlanCount(planCount + 1)} className="w-8 h-8 rounded-lg bg-white/10 border border-white/10 flex items-center justify-center hover:bg-white/20 transition text-white"><Plus size={14} /></button>
            ...
            <div className="rounded-2xl border border-white/[0.06] bg-white/[0.03] p-4 space-y-3">
              <p className="text-sm font-semibold text-white/60 flex items-center gap-2"><Clock size={14} /> Plan Activation Timing</p>
              <div className="flex items-center gap-3 flex-wrap">
                ...
                <button onClick={() => setPlanDelay({ days: 0, hours: 0, mins: 0, secs: 0 })}
                  className="text-xs px-3 py-1.5 bg-emerald-500/20 border border-emerald-500/30 text-emerald-400 rounded-lg hover:bg-emerald-500/30 transition">Instant</button>
              </div>
              <p className="text-xs text-white/25">
                {toMs(planDelay) === 0 ? "✓ Plans will appear on user dashboard immediately" : `Plans will appear after ${planDelay.days}d ${planDelay.hours}h ${planDelay.mins}m ${planDelay.secs}s`}
              </p>
            </div>
            ...
            {planSlots.some((p) => p && p.totalProfit) && (
              <div className="rounded-xl bg-emerald-500/10 border border-emerald-500/20 px-5 py-4 space-y-2">
                <div className="flex items-center justify-between">
                  ...
                  <span className="text-lg font-bold text-emerald-400">+${planSlots.reduce((s, p) => s + (parseFloat(p?.totalProfit) || 0), 0).toLocaleString()}</span>
                </div>
                <div className="flex items-center justify-between">
                  <span className="text-xs text-white/40">Combined Daily</span>
                  <span className="text-sm font-semibold text-cyan-400">+${planSlots.reduce((s, p) => s + (parseFloat(p?.dailyProfit) || 0), 0).toLocaleString()}/day</span>
                </div>
              </div>
            )}
            ...
            <button onClick={handleProcessPlans} disabled={processingPlans}
              ...
              {processingPlans ? <><Loader2 size={18} className="animate-spin" /> Processing...</> : <><TrendingUp size={18} /> Process Plans</>}
            </button>
          </div>
        )}
      </div>
